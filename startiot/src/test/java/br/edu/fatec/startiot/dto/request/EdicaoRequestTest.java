...
class EdicaoRequestTest {
    ...
    void devePassarComDadosValidos() {
        var request = new EdicaoRequest(1L, 2025, LocalDate.of(2025, 6, 15), StatusEdicao.PLANEJADA);
        assertThat(validator.validate(request)).isEmpty();
    }

    @Test
    void devePassarSemDataEvento() {
        var request = new EdicaoRequest(1L, 2025, null, StatusEdicao.PLANEJADA);
        assertThat(validator.validate(request)).isEmpty();
    }

    @Test
    void deveRejeitarEventoIdNulo() {
        var request = new EdicaoRequest(null, 2025, null, StatusEdicao.PLANEJADA);
        assertThat(campos(validator.validate(request))).contains("eventoId");
    }

    @Test
    void deveRejeitarAnoNulo() {
        var request = new EdicaoRequest(1L, null, null, StatusEdicao.PLANEJADA);
        assertThat(campos(validator.validate(request))).contains("ano");
    }

    @Test
    void deveRejeitarStatusNulo() {
        var request = new EdicaoRequest(1L, 2025, null, null);
